...
def check_demo_identity(payload: IdentityCheckRequest, db: Session = Depends(get_db)):
    ...
    if not identity:
        return IdentityCheckResponse(
            valid=False,
            message="Invalid Demo Identity. For this hackathon prototype, use synthetic IDs: 900100001234, 900100001235, 900100001236, 900100001237"
        )
...
def register_citizen(payload: RegisterCitizenRequest, request: Request, db: Session = Depends(get_db)):
    ...
    if not identity:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Invalid Demo Identity. Prototype seed numbers: 900100001234, 900100001235, 900100001236, 900100001237"
        )
    ...
    if identity.is_registered:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="This Demo Identity has already been registered. Duplicate registration rejected."
        )
